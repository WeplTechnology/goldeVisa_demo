...
function generateMockAnalysis(property, adminUserId) {
  ...
  if (priceDifferencePercent < -10) baseScore += 8
  else if (priceDifferencePercent < 0) baseScore += 5
  else if (priceDifferencePercent > 15) baseScore -= 8
  ...
  const estimatedROI = appreciationRate * 5 + (property.total_units * 0.8)
...
async function main() {
  ...
  // 1. Obtener el primer usuario admin (email @stagfund.com)
  const { data: adminUsers, error: usersError } = await supabase.rpc('get_admin_user')

  if (usersError) {
    // Si la función no existe, buscar directamente usando service role
    console.log('⚠️  Función get_admin_user no encontrada, usando consulta directa...')

    // Usar el service role key para acceder directamente a auth.users
    ...
    // Buscar el primer usuario con email @stagfund.com o simplemente el primer usuario
    let adminUser = authData.users.find(u => u.email?.endsWith('@stagfund.com'))

    if (!adminUser && authData.users.length > 0) {
      // Si no hay admin, usar el primer usuario disponible
      adminUser = authData.users[0]
      console.log('⚠️  No se encontró usuario @stagfund.com, usando primer usuario disponible')
    }
    ...
    console.log(`✅ Usuario encontrado: ${adminUser.email} (${adminUserId})\n`)

    return await processProperties(adminUserId)
  }
  ...
  await processProperties(adminUserId)
}

async function processProperties(adminUserId) {
