/**
 * Script para actualizar análisis existentes mal hechos
 * Regenera los análisis con datos profesionales
 */
...
// Función para generar análisis mock (copiada del script principal)
...
async function main() {
  console.log('🔄 Actualizando análisis existentes...\n')
  ...
  // 2. Obtener todas las propiedades con sus análisis
  ...
  // 3. Para cada análisis, regenerar con datos profesionales
  let successCount = 0
  let errorCount = 0
  ...
  for (const analysis of analyses) {
    try {
      ...
      if (propError || !property) {
        console.error(`   ❌ Error obteniendo propiedad: ${analysis.property_id}`)
        errorCount++
        continue
      }

      console.log(`🔄 Actualizando análisis: ${property.name}`)

      // Generar nuevo análisis
      const newAnalysis = generateMockAnalysis(property, adminUserId)
      ...
      if (updateError) {
        ...
        console.log(`   ✅ Análisis actualizado: Score ${newAnalysis.ai_score}/100 - ${newAnalysis.recommendation}`)
        successCount++
      }
      console.log('')
    } catch (err) {
  ...
  console.log('📊 RESUMEN')
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━')
  console.log(`✅ Análisis actualizados: ${successCount}`)
